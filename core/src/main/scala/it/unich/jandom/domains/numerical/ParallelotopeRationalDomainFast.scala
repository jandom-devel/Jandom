 ...
 */
class ParallelotopeRationalDomainFast private (favorAxis: Int) extends NumericalDomain {
  ...
  final class Property(
    ...
    type Domain = ParallelotopeRationalDomainFast

    def domain = ParallelotopeRationalDomainFast.this
    ...
    def intersectionWeak(that: Property): Property = {
      ...
      if (isEmpty)
        ...
        ParallelotopeRationalDomainFast.this.bottom(A)
      else {
        val result = that.rotate(A)
        for (i <- 0 until dimension) {
    ...
    def addVariable(): Property = {
      if (isEmpty)
        ParallelotopeRationalDomainFast.this.bottom(dimension + 1)
      else {
    ...
    def delVariable(n: Int): Property = {
      ...
      if (isEmpty)
        ParallelotopeRationalDomainFast.this.bottom(A.rows - 1)
      else {
        ...
        (set1 ++ set2).foldLeft(ParallelotopeRationalDomainFast.this.top(A.rows - 1)) { (p, lf) => p.linearInequality(lf) }
      }
    }
    ...
    def bottom = ParallelotopeRationalDomainFast.this.bottom(dimension)

    def top = ParallelotopeRationalDomainFast.this.top(dimension)
 ...
 */
object ParallelotopeRationalDomainFast {
  private lazy val standard = new ParallelotopeRationalDomainFast(0) with CachedTopBottom
  private lazy val favoring = new ParallelotopeRationalDomainFast(1) with CachedTopBottom
  private lazy val unfavoring = new ParallelotopeRationalDomainFast(-1) with CachedTopBottom
