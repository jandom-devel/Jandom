...
class ParallelotopeBreeze (
  ...
  def widening(that: ParallelotopeBreeze): ParallelotopeBreeze = {
    ...
    new ParallelotopeBreeze(false, newlow, A, newhigh)
  }

  def narrowing(that: ParallelotopeBreeze): ParallelotopeBreeze = {
    require(dimension == that.dimension)
    if (isEmpty) this
    val thatRotated = that.rotate(A)
    val newlow = low.copy
  ...
  def intersection(that: ParallelotopeBreeze): ParallelotopeBreeze = intersectionWeak(that)

  def union(that: ParallelotopeBreeze): ParallelotopeBreeze = {
    ...
    def priority(v: DenseVector[Double], ownedBy: Int = 0): PrioritizedConstraint = {
      val y1 = A.t \ v
      val (l1, u1) = ParallelotopeBreeze.extremalsInBox(y1, low, high)
      val y2 = that.A.t \ v
      val (l2, u2) = ParallelotopeBreeze.extremalsInBox(y2, that.low, that.high)
      val p =
    ...
    val pvt = ParallelotopeBreeze.pivoting(Qsorted map (_._1))
    return new ParallelotopeBreeze(false, toDenseVector(newlow(pvt)),
        toDenseMatrix(newA(pvt, ::)), toDenseVector(newhigh(pvt)))
  }

  def unionWeak(that: ParallelotopeBreeze): ParallelotopeBreeze = {
    require(dimension == that.dimension)
  ...
  def intersectionWeak(that: ParallelotopeBreeze): ParallelotopeBreeze = {
    require(dimension == that.dimension)
    val result = that.rotate(A)
    for (i <- 0 to dimension - 1) {
  ...
  def linearAssignment(n: Int, tcoeff: Array[Double], known: Double): ParallelotopeBreeze = {
    ...
    if (coeff(n) != 0) {
      ...
      new ParallelotopeBreeze(false, newlow, newA, newhigh)
    } else {
  ...
  def linearInequality(tcoeff: Array[Double], known: Double): ParallelotopeBreeze = {
    ...
    j match {
      case None => {
        val newlow = low.copy
        val newhigh = high.copy
        val (minc, maxc) = ParallelotopeBreeze.extremalsInBox(y, newlow, newhigh)
        if (minc > -known) return ParallelotopeBreeze.empty(dimension)
        for (i <- 0 to dimension - 1) {
          if (y(i) > 0)
        ...
        return new ParallelotopeBreeze(false, newlow, A, newhigh)
      }
      case Some(j) => {
        val newA = A.copy
        val newhigh = high.copy
        ...
        newhigh(j) = -known
        return new ParallelotopeBreeze(false, low, newA, newhigh)
      }
    }
  }

  def linearDisequality(coeff: Array[Double], known: Double): ParallelotopeBreeze = {
   ...
   */
  def nondeterministicAssignment(n: Int): ParallelotopeBreeze = {
    require(n <= dimension)
    if (isEmpty) return this;
  ...
  def empty = ParallelotopeBreeze.empty(dimension)

  def full = ParallelotopeBreeze.full(dimension)

  def tryCompareTo[B >: ParallelotopeBreeze](that: B)(implicit arg0: (B) => PartiallyOrdered[B]): Option[Int] = that match {
    case that: ParallelotopeBreeze =>
      val lte = this <= that
      val gte = that <= this
      if (lte && gte)
        Some(0)
   ...
   */
  def rotate(Aprime: DenseMatrix[Double]): ParallelotopeBreeze = {
    ...
    return new ParallelotopeBreeze(false, newlow, Aprime, newhigh)
  }

  def <=[B >: ParallelotopeBreeze](that: ParallelotopeBreeze)(implicit arg0: (B) ⇒ PartiallyOrdered[B]): Boolean = {
  ...
  override def equals(other: Any): Boolean = other match {
    case other: ParallelotopeBreeze => (this <= other) && (other <= this)
    case _ => false
  }
...
object ParallelotopeBreeze extends NumericalDomain[ParallelotopeBreeze] with ParameterValue {
  ...
  def apply(low: DenseVector[Double], A: DenseMatrix[Double], high: DenseVector[Double]) =
    new ParallelotopeBreeze(false, low, A, high)
  ...
  def empty(n: Int) = {
    ...
    val A = DenseMatrix.eye[Double](n)
    new ParallelotopeBreeze(true, low, A, high)
  }
